...
class PostDense(nn.Module):
    def __init__(self, n_in, k, stride = 1):
        super(PostDense, self).__init__()
        self.conv1 = nn.Conv3d(n_in, k, kernel_size=3, stride=stride, padding=1)
        self.bn1 = nn.BatchNorm3d(k)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv3d(n_in+k, k, kernel_size=3, stride=stride, padding=1)
        self.bn2 = nn.BatchNorm3d(k)

        if stride !=1:
            self.shortcut = nn.Sequential(
                nn.Conv3d(n_in, n_in, kernal_size=1, stride=stride),
                nn.BatchNorm3d(n_in))
        else:
            self.shortcut = None
    ...
    def forward(self, x):
        dense = x
        if self.shortcut is not None:
            dense = self.shortcut(x)
        out = self.conv1(x)
        out = self.bn1(out)
        out = self.relu(out)
        out = torch.cat((dense, out), 1)
...
class SEBlock(nn.Module):
    def __init__(self, n_in, stride=16):
        super(SEBlock, self).__init__()
        #assert n_in%stride == 0, "n_in must be divisible by %d" % stride
        #remainder = n_in%stride
        #n_out = (n_in-remainder)%stride
        self.glp = nn.AdaptiveAvgPool3d(1)
        self.fc1 = nn.Linear(n_in, n_in/stride)
        self.relu = nn.ReLU(inplace=True)
        self.fc2 = nn.Linear(n_in/stride, n_in)
    ...
    def forward(self, x):
        ...
        #print "sigmoid size", out.size()
        out = out.view(-1,self.n_in,1,1,1)
        out = torch.mul(x, out)
        #print "multiply size", out.size()
        return out
...
class Loss_fpn(nn.Module):
    ...
    def forward(self, outputs, labels, train=True):
        batch_size = labels[0].size(0)
        #print "output0.size", outputs[0].size()
        #print "output1.size", outputs[1].size()
        output0 = outputs[0].view(-1, 5)
        output1 = outputs[1].view(-1, 5)
        labels0 = labels[0].view(-1, 5)
        labels1 = labels[1].view(-1, 5)

        pos_idcs0 = labels0[:, 0] > 0.5
        #print "pos_idcs0.size()", pos_idcs0.size()
        pos_idcs1 = labels1[:, 0] > 0.5
        #print "pos_idcs1.size()", pos_idcs1.size()
        pos_idcs0 = pos_idcs0.unsqueeze(1).expand(pos_idcs0.size(0), 5)
        pos_idcs1 = pos_idcs1.unsqueeze(1).expand(pos_idcs1.size(0), 5)
        pos_output0 = output0[pos_idcs0].view(-1, 5)
        pos_output1 = output1[pos_idcs1].view(-1, 5)
        pos_labels0 = labels0[pos_idcs0].view(-1, 5)
        pos_labels1 = labels1[pos_idcs1].view(-1, 5)

        neg_idcs0 = labels0[:, 0] < -0.5
        neg_idcs1 = labels1[:, 0] < -0.5
        neg_output0 = output0[:, 0][neg_idcs0]
        neg_output1 = output1[:, 0][neg_idcs1]
        neg_labels0 = labels0[:, 0][neg_idcs0]
        neg_labels1 = labels1[:, 0][neg_idcs1]

        if self.num_hard > 0 and train:
            neg_output0, neg_labels0 = hard_mining(neg_output0, neg_labels0, self.num_hard * batch_size)
            neg_output1, neg_labels1 = hard_mining(neg_output1, neg_labels1, self.num_hard * batch_size)
        neg_prob0 = self.sigmoid(neg_output0)
        neg_prob1 = self.sigmoid(neg_output1)

        # classify_loss = self.classify_loss(
        #   torch.cat((pos_prob, neg_prob), 0),
        #  torch.cat((pos_labels[:, 0], neg_labels + 1), 0))
        if len(pos_output0) > 0:
            pos_prob0 = self.sigmoid(pos_output0[:, 0])
            pz, ph, pw, pd = pos_output0[:, 1], pos_output0[:, 2], pos_output0[:, 3], pos_output0[:, 4]
            lz, lh, lw, ld = pos_labels0[:, 1], pos_labels0[:, 2], pos_labels0[:, 3], pos_labels0[:, 4]
            ...
            regress_losses0 = [
                ...
                self.regress_loss(pw, lw),
                self.regress_loss(pd, ld)]
            regress_losses_data0 = [l.data[0] for l in regress_losses0]
            classify_loss0 = 0.5 * self.classify_loss(
                pos_prob0, pos_labels0[:, 0]) + 0.5 * self.classify_loss(
                neg_prob0, neg_labels0 + 1)
            pos_correct0 = (pos_prob0.data >= 0.5).sum()
            pos_total0 = len(pos_prob0)

        else:
            regress_losses0 = [0, 0, 0, 0]
            classify_loss0 = 0.5 * self.classify_loss(
                neg_prob0, neg_labels0 + 1)
            pos_correct0 = 0
            pos_total0 = 0
            regress_losses_data0 = [0, 0, 0, 0]
        classify_loss_data0 = classify_loss0.data[0]

        if len(pos_output1) > 0:
            pos_prob1 = self.sigmoid(pos_output1[:, 0])
            pz, ph, pw, pd = pos_output1[:, 1], pos_output1[:, 2], pos_output1[:, 3], pos_output1[:, 4]
            lz, lh, lw, ld = pos_labels1[:, 1], pos_labels1[:, 2], pos_labels1[:, 3], pos_labels1[:, 4]
            ...
            regress_losses1 = [
                ...
                self.regress_loss(pw, lw),
                self.regress_loss(pd, ld)]
            regress_losses_data1 = [l.data[0] for l in regress_losses1]
            classify_loss1 = 0.5 * self.classify_loss(
                pos_prob1, pos_labels1[:, 0]) + 0.5 * self.classify_loss(
                neg_prob1, neg_labels1 + 1)
            pos_correct1 = (pos_prob1.data >= 0.5).sum()
            pos_total1 = len(pos_prob1)

        else:
            regress_losses1 = [0, 0, 0, 0]
            classify_loss1 = 0.5 * self.classify_loss(
                neg_prob1, neg_labels1 + 1)
            pos_correct1 = 0
            pos_total1 = 0
            regress_losses_data1 = [0, 0, 0, 0]
        classify_loss_data1 = classify_loss1.data[0]

        loss = classify_loss0 + classify_loss1
        for regress_loss0 in regress_losses0:
            loss += regress_loss0
        for regress_loss1 in regress_losses1:
            loss += regress_loss1
        loss /= 2.

        neg_correct = (neg_prob0.data < 0.5).sum()
        neg_total = len(neg_prob0)

        return [loss, classify_loss_data0] + regress_losses_data0 + [pos_correct0, pos_total0, neg_correct, neg_total]

# get the proposed bounding box
class GetPBB(object):
...
# filter out the unqualified bounding boxes
